...
import { useArchivedCategoriesQuery } from '@/queries/useArchivedCategoriesQuery'
import {
  Button,
  ConfirmModal,
  EmptyState,
...
export default function ArchivedCategoriesModal({
  walletId,
  open,
  onOpenChange,
  ...
  const archivedQuery = useArchivedCategoriesQuery(walletId, open)

  const [confirmOpen, setConfirmOpen] = useState(false)
  const [toHardDelete, setToHardDelete] = useState<{ id: string; name: string } | null>(null)

  const hardDeleteMutation = useMutation({
    mutationFn: (categoryId: string) => categoriesApi.hardDelete(walletId, categoryId),
    onSuccess: () => {
      notify.success('Category permanently deleted')

      queryClient.invalidateQueries({ queryKey: queryKeys.wallets.categories.all(walletId) })
      queryClient.invalidateQueries({ queryKey: queryKeys.wallets.categories.archived(walletId) })

      setToHardDelete(null)
      setConfirmOpen(false)
    },
    onError: (err) => {
      notify.fromError(err, 'Failed to hard delete category')
    },
  })
